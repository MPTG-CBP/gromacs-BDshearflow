...
static void do_update_bd(int                  start,
			 ...
			 real                 gamma, //shear flow
			 real                 z0, //z for which vflow=0
			 const real*          rf,
                         int64_t              step,
                         int                  seed,
                         const int*           gatindex)
{
    ...
    for (n = start; (n < nrend); n++)
    {
        ...
        for (d = 0; (d < DIM); d++)
        {
		  ...
		  // contribution to the displacement (in x axis) due to the shear flow (in z axis)
		if(d==0)
		xprime[n][d]+= gamma*(x[n][2] - z0 )*dt ; 
            ...
            switch (inputrec->eI)
            {
                ...
                case (eiBD):
                    do_update_bd(start_th, end_th, dt, inputrec->opts.nFreeze, md->invmass,
                                 ...
                                 inputrec->bd_fric, inputrec->bd_gamma, inputrec->bd_z0, upd->sd()->bd_rf.data(), step, inputrec->ld_seed,
                                 DOMAINDECOMP(cr) ? cr->dd->globalAtomIndices.data() : nullptr);     
                    break;
                case (eiVV):
                case (eiVVAK):
